...
class TwoStageFounderAnalysis:
    ...
    def preprocess_data(self, df):
        ...
        target_n_failure = int(n_success * (1 - self.real_world_success_rate) / self.real_world_success_rate)
    ...
    def create_subclusters(self, X, df, main_cluster_labels):
        ...
        for cluster in range(self.n_main_clusters):
            ...
            importances = dict(zip(self.feature_names, tree.feature_importances_))
            self.feature_importances[cluster + 1] = {
                feat: imp for feat, imp in importances.items()
                if imp > 0
            }
    ...
    def save_summary_tables(self, main_summary, sub_summary):
        ...
        with pd.ExcelWriter('founder_clusters_analysis.xlsx', engine='xlsxwriter') as writer:
            ...
            feature_importance_format = workbook.add_format({
                'italic': True,
                'bg_color': '#F0F8FF',
                'text_wrap': True,
                'valign': 'vcenter',
            ...
            for row_num in range(1, len(hierarchical_df) + 1):
                ...
                format_to_use = main_format if hierarchical_df.iloc[row_num - 1]['Cluster Level'] == 'Main' else \
                    feature_importance_format if hierarchical_df.iloc[row_num - 1][
                                                     'Cluster Level'] == 'Feature Importance' else \
                        sub_format
    ...
    def generate_hierarchical_table(self, main_summary, sub_summary):
        ...
        for _, main_cluster in sorted_main.iterrows():
            ...
            if main_cluster['Cluster ID'] in self.feature_importances:
                importances = self.feature_importances[main_cluster['Cluster ID']]
                importance_str = '\n'.join([
                    f"{feat}: {imp:.3f}"
                ...
                hierarchical_rows.append({
                    ...
                    'Relative % of Success': '',
                    'Relative % of Total': '',
                    'Key Characteristics': importance_str,
                    'Decision Path': ''
                })
...
df = pd.read_csv("after_embedding.csv")
